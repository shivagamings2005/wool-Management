            ...
            public void actionPerformed(ActionEvent e){
                if (!ust.getText().isEmpty() && !new String(passt.getPassword()).isEmpty() && !new String(repasst.getPassword()).isEmpty() && !addrt.getText().isEmpty() && !pht.getText().isEmpty() && !aadt.getText().isEmpty()) {
                    try{
                        ...
                        sq.st.executeUpdate("create table if not exists woolprocess(id int primary key auto_increment,name char(20),passwd char(20),address char(40),phone char(13),aadhar char(12),exp int,availablity char(3) default 'yes')");
                        sq.res=sq.st.executeQuery("select count(name) from woolprocess where name='"+ust.getText()+"'");
                        sq.res.next();
                            ...
                            if(new String(passt.getPassword()).equals(new String(repasst.getPassword()))){
                                if(pht.getText().length()==10 || pht.getText().length()==12 || pht.getText().length()==13){
                                    if(aadt.getText().length()==12 ){
                                        sq.st.executeUpdate("insert into woolprocess(name,passwd,exp,address,phone,aadhar) values('"+ust.getText()+"','"+new String(passt.getPassword())
                                        +"',"+Integer.parseInt(expt.getText())+",'"+addrt.getText()+"','"+pht.getText()+"','"+aadt.getText()+"')");
                                        new wpproc(ust.getText());
                                        f.dispose();
                                    }
                                    else{
                                        JOptionPane.showMessageDialog(null,"Invalid aadhaar number","NOTE",JOptionPane.INFORMATION_MESSAGE);    
                                        aadt.setText("");
                                    }
                                }
                                else{
